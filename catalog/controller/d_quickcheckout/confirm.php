...
class ControllerDQuickcheckoutConfirm extends Controller {
    ...
    public function updateOrder(){
		...
		if(empty($order_data['lastname']) && !empty($order_data['firstname'])) {
			$names = explode(' ', $order_data['firstname']);
			$order_data['firstname'] = '';
			
			for($i=0;$i<count($names)-1;$i++){
				$order_data['firstname'] .= $names[$i].' ';
			}
			
			$order_data['lastname'] = $names[count($names)-1];
		}
		
		$order_data['firstname'] = preg_replace("/\s+$/m", "", $order_data['firstname']);
		$order_data['lastname'] = preg_replace("/\s+$/m", "", $order_data['lastname']);
		...
		if(empty($order_data['payment_lastname']) && !empty($order_data['payment_firstname'])) {
			$names = explode(' ', $order_data['payment_firstname']);
			$order_data['payment_firstname'] = '';
			
			for($i=0;$i<count($names)-1;$i++){
				$order_data['payment_firstname'] .= $names[$i].' ';
			}
			
			$order_data['payment_lastname'] = $names[count($names)-1];
		}
		
		$order_data['payment_firstname'] = preg_replace("/\s+$/m", "", $order_data['payment_firstname']);
		$order_data['payment_lastname'] = preg_replace("/\s+$/m", "", $order_data['payment_lastname']);
		
		if(empty($order_data['payment_city'])) $order_data['payment_city'] = $order_data['payment_zone'];
		if(empty($order_data['payment_postcode'])) $order_data['payment_postcode'] = '000000';
        ...
        if ($this->cart->hasShipping()) {
			...
			if(empty($order_data['shipping_lastname']) && !empty($order_data['shipping_firstname'])) {
				$names = explode(' ', $order_data['shipping_firstname']);
				$order_data['shipping_firstname'] = '';
				
				for($i=0;$i<count($names)-1;$i++){
					$order_data['shipping_firstname'] .= $names[$i].' ';
				}
				
				$order_data['shipping_lastname'] = $names[count($names)-1];
			}
			
			$order_data['shipping_firstname'] = preg_replace("/\s+$/m", "", $order_data['shipping_firstname']);
			$order_data['shipping_lastname'] = preg_replace("/\s+$/m", "", $order_data['shipping_lastname']);
			
			if(empty($order_data['shipping_city'])) $order_data['shipping_city'] = $order_data['shipping_zone'];
			if(empty($order_data['shipping_postcode'])) $order_data['shipping_postcode'] = '000000';
