...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_xemthem'] = $this->language->get('text_xemthem');
			$data['text_thugon'] = $this->language->get('text_thugon');
			...
			$data['reviews2'] = (int)$product_info['reviews'];
			$data['rating'] = (float)$product_info['rating'];
			$all_rating = $this->model_catalog_review->getAllReviewsByProductId($this->request->get['product_id']);
			$data['rating5'] = 0;
			$data['rating4'] = 0;
			$data['rating3'] = 0;
			$data['rating2'] = 0;
			$data['rating1'] = 0;
			foreach ($all_rating as $key_all_rating => $value_all_rating) {
				if($value_all_rating['rating'] == 5){
					$data['rating5']++;
				}
				if($value_all_rating['rating'] == 4){
					$data['rating4']++;
				}
				if($value_all_rating['rating'] == 3){
					$data['rating3']++;
				}
				if($value_all_rating['rating'] == 2){
					$data['rating2']++;
				}
				if($value_all_rating['rating'] == 1){
					$data['rating1']++;
				}
			}
