...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			$data['text_phanloai'] = $this->language->get('text_phanloai');
			$data['text_tatca'] = $this->language->get('text_tatca');
			$data['text_sanpham'] = $this->language->get('text_sanpham');
			$data['text_boloc'] = $this->language->get('text_boloc');
			...
			if ($category_info['image_banner']) {
				$data['thumb_banner'] = $this->model_tool_image->resize($category_info['image_banner'], 1900, 350);
			} else if($this->config->get('config_category')) {
				$data['thumb_banner'] = $this->model_tool_image->resize($this->config->get('config_category'), 1900, 350);
			} else {
				$data['thumb_banner'] = $this->model_tool_image->resize('category-default3-1900x350.jpg', 1900, 350);
			}

			if ($category_info['image_banner_mobile']) {
				$data['thumb_banner_mobile'] = $this->model_tool_image->resize($category_info['image_banner_mobile'], 750, 730);
			} else if($this->config->get('config_category_mobile')) {
				$data['thumb_banner_mobile'] = $this->model_tool_image->resize($this->config->get('config_category_mobile'), 750, 730);
			} else {
				$data['thumb_banner_mobile'] = $this->model_tool_image->resize('category-default3-750x730.jpg', 750, 730);
			}
