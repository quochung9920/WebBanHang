	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<% if(model.account == 'logged'){ %> 
		        ...
		        <form id="payment_address_form" autocomplete="off" class="form-horizontal <%= model.payment_address.address_id == 'new' ? '' : 'hidden' %>">
				
				</form>

			<% }else{ %>
			<form id="payment_address_form" autocomplete="off" class="form-horizontal">
...
<script>
jQuery(function() {
	qc.paymentAddress = jQuery.extend(true, {}, new qc.PaymentAddress(<?php echo $json; ?>));
	qc.paymentAddressView = jQuery.extend(true, {}, new qc.PaymentAddressView({
		el:jQuery("#payment_address"), 
		model: qc.paymentAddress, 
		template: _.template(jQuery("#payment_address_template").html())
	}));
