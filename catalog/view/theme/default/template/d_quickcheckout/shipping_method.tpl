...
<form id="shipping_method_form" autocomplete="off" <%= parseInt(model.config.display) && model.show_shipping_method ? '' : 'class="hidden"' %>>
   ...
   $(document).ready(function(){
        if($("#shipping_method_list > div").hasClass("1")){
          	$("#shipping_method_list > div.1").children().children().trigger("click");
          	$("#shipping_method_list > div.1").next().css("display","none");
          	$("#shipping_method_list > div.0").css("display","none");
        }
   });
</script>
</form>
...
<script>
jQuery(function() {
	qc.shippingMethod = jQuery.extend(true, {}, new qc.ShippingMethod(<?php echo $json; ?>));
	qc.shippingMethodView = jQuery.extend(true, {}, new qc.ShippingMethodView({
		el:jQuery("#shipping_method"), 
		model: qc.shippingMethod, 
		template: _.template(jQuery("#shipping_method_template").html())
	}));
});
</script>