	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<% if(model.account == 'logged'){ %> 
		        ...
		        <form id="shipping_address_form" autocomplete="off" class="form-horizontal <%= model.shipping_address.address_id == 'new' ? '' : 'hidden' %>">
				
				</form>

			<% }else{ %>
			<form id="shipping_address_form" autocomplete="off" class="form-horizontal">
...
jQuery(function() {
	qc.shippingAddress = jQuery.extend(true, {}, new qc.ShippingAddress(<?php echo $json; ?>));
	qc.shippingAddressView = jQuery.extend(true, {}, new qc.ShippingAddressView({
		el:jQuery("#shipping_address"), 
		model: qc.shippingAddress, 
		template: _.template(jQuery("#shipping_address_template").html())
	}));
