...
class ModelUserUser extends Model {
	...
	public function editPassword($user_id, $password) {
		$this->db->query("UPDATE `" . DB_PREFIX . "user` SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($password)))) . "', code = '', last_activity = NOW() WHERE user_id = '" . (int)$user_id . "'");
	}
	...
	public function getUsers($data = array()) {
		...
		if(!$this->user->isRoot()) $sql .= " WHERE root='0'";
