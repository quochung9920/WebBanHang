...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		$data['entry_logo_share_fb'] = 'Logo share facebook/zalo...';
		...
		if (isset($this->request->post['config_category'])) {
			...
			$data['config_category'] = $this->config->get('config_category');
		}

		if (isset($this->request->post['config_category']) && is_file(DIR_IMAGE . $this->request->post['config_category'])) {
			$data['category'] = $this->model_tool_image->resize($this->request->post['config_category'], 100, 100);
		} elseif ($this->config->get('config_category') && is_file(DIR_IMAGE . $this->config->get('config_category'))) {
			$data['category'] = $this->model_tool_image->resize($this->config->get('config_category'), 100, 100);
		} else {
			$data['category'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}

		if (isset($this->request->post['config_category_mobile'])) {
			$data['config_category_mobile'] = $this->request->post['config_category_mobile'];
		} else {
			$data['config_category_mobile'] = $this->config->get('config_category_mobile');
		}

		if (isset($this->request->post['config_category_mobile']) && is_file(DIR_IMAGE . $this->request->post['config_category_mobile'])) {
			$data['category_mobile'] = $this->model_tool_image->resize($this->request->post['config_category_mobile'], 100, 100);
		} elseif ($this->config->get('config_category_mobile') && is_file(DIR_IMAGE . $this->config->get('config_category_mobile'))) {
			$data['category_mobile'] = $this->model_tool_image->resize($this->config->get('config_category_mobile'), 100, 100);
		} else {
			$data['category_mobile'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}


		if (isset($this->request->post['config_logo_share_fb'])) {
			$data['config_logo_share_fb'] = $this->request->post['config_logo_share_fb'];
		} else {
			$data['config_logo_share_fb'] = $this->config->get('config_logo_share_fb');
		}

		if (isset($this->request->post['config_logo_share_fb']) && is_file(DIR_IMAGE . $this->request->post['config_logo_share_fb'])) {
			$data['logo_share_fb'] = $this->model_tool_image->resize($this->request->post['config_logo_share_fb'], 100, 100);
		} elseif ($this->config->get('config_logo_share_fb') && is_file(DIR_IMAGE . $this->config->get('config_logo_share_fb'))) {
			$data['logo_share_fb'] = $this->model_tool_image->resize($this->config->get('config_logo_share_fb'), 100, 100);
		} else {
			$data['logo_share_fb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
