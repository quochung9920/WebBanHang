		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$data['orders'][] = array(
					...
					'store_email'        => $store_email[$this->config->get('config_language_id')],
					'store_telephone'    => $store_telephone[$this->config->get('config_language_id')],
					'store_fax'          => $store_fax[$this->config->get('config_language_id')],
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info && $order_info['shipping_code']) {
				...
				$data['orders'][] = array(
					...
					'store_name'       => $order_info['store_name'],
					'store_url'        => rtrim($order_info['store_url'], '/'),
					'store_address'    => nl2br($store_address[$this->config->get('config_language_id')]),
					'store_email'      => $store_email[$this->config->get('config_language_id')],
					'store_telephone'  => $store_telephone[$this->config->get('config_language_id')],
					'store_fax'        => $store_fax[$this->config->get('config_language_id')],
					'email'            => $order_info['email'],
